...
impl AppConfig {
    ...
    /// Returns true if `device_id` is the radiator fan cluster owned by an AIO.
    pub fn is_radiator_owned_by_aio(&self, device_id: &str) -> bool {
        self.aio
            .values()
            .any(|cfg| cfg.radiator_fan_device_id.as_deref() == Some(device_id))
    }

    /// Drop a standalone fan group for a radiator cluster now driven via AIO config.
    pub fn remove_fan_group_for_device(&mut self, device_id: &str) -> bool {
        let Some(fans) = self.fans.as_mut() else {
            return false;
        };
        let before = fans.speeds.len();
        fans.speeds
            .retain(|g| g.device_id.as_deref() != Some(device_id));
        fans.speeds.len() < before
    }
}
