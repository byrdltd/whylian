...
/// Install a template from the shipped `templates/` tree (no network).
pub fn install_bundled_template(
    template_id: &str,
    bundled_root: &std::path::Path,
    sensors: &[SensorInfo],
) -> Result<LcdTemplate> {
    let manifest_path = bundled_root.join("default_templates.json");
    let manifest_json = std::fs::read_to_string(&manifest_path)
        .with_context(|| format!("reading {}", manifest_path.display()))?;
    let manifest: CatalogManifest =
        serde_json::from_str(&manifest_json).context("parsing bundled manifest")?;
    let template = manifest
        .templates
        .into_iter()
        .find(|t| t.id == template_id)
        .ok_or_else(|| anyhow!("bundled template '{template_id}' not found"))?;

    let assets_root = bundled_root.join("assets").join(&template.folder);
    let install_root = templates_install_dir()?;
    let target_dir = install_root.join(&template.id);
    let staging_dir = install_root.join(format!(".{}.staging", template.id));
    if staging_dir.exists() {
    ...
    let tpl_path = assets_root.join(&template.template_file);
    let tpl_bytes =
        std::fs::read(&tpl_path).with_context(|| format!("reading {}", tpl_path.display()))?;
    verify_sha256(&tpl_bytes, &template.template_sha256)
        .context("template.json sha256 mismatch")?;
    std::fs::write(staging_dir.join(&template.template_file), &tpl_bytes)
        .context("writing staged template.json")?;
    ...
    for file in &template.files {
        let src = assets_root.join(&file.path);
    ...
    let mut lcd_template: LcdTemplate = serde_json::from_slice(&tpl_bytes)
        .context("parsing bundled template.json after sha256 verify")?;
    rewrite_asset_paths(&mut lcd_template, &target_dir);
    resolve_sensor_categories(&mut lcd_template, sensors);
    lcd_template.validate().map_err(|e| anyhow!("{e}"))?;
    ...
    info!(
        "installed bundled template '{}' from {}",
        template.id,
...
/// Search paths for the shipped template catalog (newest first).
pub fn bundled_templates_roots() -> Vec<PathBuf> {
    let mut roots = Vec::new();
    if let Ok(dir) = std::env::var("WHYLIAN_TEMPLATES_DIR") {
    ...
    {
        roots.push(data_home.join("whylian/templates"));
    }
    roots.push(PathBuf::from("/usr/share/whylian/templates"));
    if let Ok(exe) = std::env::current_exe() {
        if let Some(parent) = exe.parent() {
            roots.push(parent.join("../share/whylian/templates"));
        }
    }
    roots
}

pub fn find_bundled_templates_root() -> Option<PathBuf> {
    bundled_templates_roots()
        .into_iter()
        .find(|p| p.join("default_templates.json").is_file())
}
