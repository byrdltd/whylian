...
use lianli_devices::wireless::{SharedWirelessController, WirelessFanType};
...
impl RgbController {
    ...
    pub fn set_effect(
            ...
            (&self.wireless, self.wireless_state.get_mut(device_id))
        {
            ...
            wc.send_rgb_direct(&state.mac, &state.led_state, &idx, 4)?;
            debug!(
    ...
    pub fn set_direct_colors(
            ...
            (&self.wireless, self.wireless_state.get_mut(device_id))
        {
            ...
            let idx = effect_index_from_state(&state.led_state);
            wc.send_rgb_direct(&state.mac, &state.led_state, &idx, 2)?;
            return Ok(());
        }
    ...
    pub fn set_wireless(&mut self, wireless: Option<SharedWirelessController>) {
