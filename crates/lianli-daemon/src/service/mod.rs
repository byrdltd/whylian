...
use lianli_devices::wireless::{SharedWirelessController, WirelessController};
...
impl ServiceManager {
    pub fn new(config_path: PathBuf) -> Result<Self> {
        ...
        Ok(Self {
            ...
            targets: HashMap::new(),
            wireless: Arc::new(Mutex::new(WirelessController::new())),
    ...
    pub fn device_poll(&mut self) {
        ...
        if wall_elapsed > mono_elapsed + Duration::from_secs(5) {
            info!(
                "System resume detected (~{:.0}s sleep), recovering wireless and restarting",
        ...
        let current_wireless = self.wireless.lock().devices().len();
        if current_wireless != self.last_wireless_count {
            if current_wireless > self.last_wireless_count {
                info!(
                    "Wireless device count changed ({} -> {}), rebuilding controllers",
    ...
    pub fn run(&mut self) -> Result<bool> {
        ...
        info!("=====================================================================");
        warn!(
            "Operator assumes all hardware/cooling risk — no warranty; see DISCLAIMER.md \
             (https://github.com/byrdltd/whyLIAN/blob/main/DISCLAIMER.md)"
        );
        ...
        self.try_wireless();
        self.last_wireless_count = self.wireless.lock().devices().len();
        if !self.use_rusb() {
        ...
        for event in rx {
            match event {
                ...
                DaemonEvent::USBCheck => {
                    ...
                    if !self.wireless.lock().is_connected() {
                        self.try_wireless();
                    }
                ...
                DaemonEvent::Bind {
                    ...
                    if let Some(mac) = parse_mac_str(&mac_str) {
                        if let Err(e) = self.wireless.lock().bind_device(&mac) {
                ...
                DaemonEvent::Unbind {
                    ...
                    if let Some(mac) = parse_mac_str(&mac_str) {
                        if let Err(e) = self.wireless.lock().unbind_device(&mac) {
