...
/// Install the default `cooler` LCD template from the shipped catalog when missing.
pub fn ensure_bundled_cooler_template(
    templates_path: &Path,
    sensors: &[SensorInfo],
) -> Option<Vec<LcdTemplate>> {
    let mut user = load_user_templates(templates_path);
    if user.iter().any(|t| t.id == "cooler") {
        return None;
    }
    let bundled = lianli_shared::template_catalog::find_bundled_templates_root().or_else(|| {
        let dev = PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../../templates"));
        dev.join("default_templates.json").is_file().then_some(dev)
    });
    let bundled = match bundled {
        Some(p) => p,
        None => return None,
    };
    match lianli_shared::template_catalog::install_bundled_template("cooler", &bundled, sensors) {
        Ok(tpl) => {
            user.push(tpl);
            if let Err(e) = save_user_templates(templates_path, &user) {
                warn!("Failed to persist bundled cooler template: {e}");
            }
            regenerate_template_previews(&user, sensors);
            info!("Registered bundled cooler LCD template");
            Some(user)
        }
        Err(e) => {
            warn!("Bundled cooler template install failed: {e:#}");
            None
        }
    }
}
