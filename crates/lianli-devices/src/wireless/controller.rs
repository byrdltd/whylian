...
pub struct WirelessController {
    ...
    /// Set after two consecutive successful RX polls (see `start_polling`).
    discovery_stable: Arc<AtomicBool>,
}
...
impl WirelessController {
    pub fn new() -> Self {
        Self {
            ...
            discovery_stable: Arc::new(AtomicBool::new(false)),
        }
    }

    /// True after RX discovery has been stable for two consecutive polls.
    pub fn is_discovery_stable(&self) -> bool {
        self.discovery_stable.load(Ordering::Acquire)
    }

    /// Block until discovery is stable or `timeout` elapses.
    pub fn wait_for_discovery_stable(&self, timeout: StdDuration) -> bool {
        let deadline = std::time::Instant::now() + timeout;
        while std::time::Instant::now() < deadline {
            if self.is_discovery_stable() {
                return true;
            }
            thread::sleep(Duration::from_millis(50));
        }
        self.is_discovery_stable()
    }

    /// Wireless TX is up and at least one bound device has been discovered.
    pub fn is_ready_for_control(&self) -> bool {
        self.is_connected() && self.has_discovered_devices()
    }
    ...
    pub fn connect(&mut self) -> Result<()> {
        ...
        self.discovery_stable.store(false, Ordering::Release);
    ...
    pub fn start_polling(&mut self) -> Result<()> {
        ...
        self.discovery_stable.store(false, Ordering::Release);
        let discovery_signal = Arc::clone(&self.discovery_stable);
        ...
        if self.wait_for_discovery_stable(Duration::from_secs(15)) {
            info!("Wireless discovery stable, proceeding with device list");
        } else if self.has_discovered_devices() {
            warn!(
                "Wireless discovery not yet stable but {} device(s) present — proceeding",
                self.discovered_device_count()
            );
        } else {
            warn!(
                "Wireless discovery timed out (15s) — controllers will start when devices appear"
            );
        }
        Ok(())
    }
